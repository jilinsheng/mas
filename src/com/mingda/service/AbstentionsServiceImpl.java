...
public class AbstentionsServiceImpl implements AbstentionsService{
	static Logger log = Logger.getLogger(AbstentionsServiceImpl.class);
	
	private ExtendsDAO extendsDAO;
	private Pager pager;
	private SanwufileDAO sanwufileDAO;
	private WubaohuDAO wubaohuDAO;
	
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public List<WubaohuDTO> findAbstentions(String sql, int currentpage,
			String url) {
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public WubaohuDTO saveAbstentions(WubaohuDTO wubaohuDTO) {
	...
	@Override
	public void saveSanwufiles(List<SanwufileDTO> files){
		for (SanwufileDTO s : files) {
			Sanwufile e = new Sanwufile();
			e.setBizId(s.getBizId());
			e.setFilename(s.getFilename());
			e.setRealfilename(s.getRealfilename());
			e.setRealpath(s.getRealpath());
			sanwufileDAO.insert(e);
		}
	}
	
	@Override
	public List<SanwufileDTO> findSanwufiles(String bizId) {
		List<SanwufileDTO> list = new ArrayList<SanwufileDTO>();
		SanwufileExample example = new SanwufileExample();
		example.createCriteria().andBizIdEqualTo(new BigDecimal(bizId));
		List<Sanwufile> rs = sanwufileDAO
				.selectByExample(example);
		for (Sanwufile s : rs) {
			SanwufileDTO e = new SanwufileDTO();
			e.setBizId(s.getBizId());
			e.setFilename(s.getFilename());
			e.setRealfilename(s.getRealfilename());
			e.setRealpath(s.getRealpath());
	...
	public WubaohuDTO findSanwuById(String id,String ds) {
		WubaohuDTO e = new WubaohuDTO();
	...
	public void delSanwuFile(String fid) {
		SanwufileExample example = new SanwufileExample();
		example.createCriteria().andFileIdEqualTo(new BigDecimal(fid));
		this.sanwufileDAO.deleteByExample(example);
	}
	...
	public SanwufileDAO getSanwufileDAO() {
		return sanwufileDAO;
	}

	public void setSanwufileDAO(SanwufileDAO sanwufileDAO) {
		this.sanwufileDAO = sanwufileDAO;
	}
