...
<script type="text/javascript">
	...
	function valpaperid(obj) {
		$.ajax({
			...
			data : {
				"paperId" : obj.value,
				"memberId" : $("#editorphan_wubaohuDTO_memberId")[0].value
			},
			timeout : 20000,
			error : function() {
				alert("服务器错误");
	...
	function add(){
		temp='<input name="orphan" type="file" id="file1" size="40"/>&nbsp;&nbsp;<img style="padding-right:2px" src="<%=path%>/page/images/del.gif" onclick ="delfile(this)"></img>';
		var newRadioButton = document.createElement("div");
		dfile1.insertBefore(newRadioButton);
		newRadioButton.innerHTML=temp;
	}
	function valfile(){
		var temps =document.getElementsByName('orphan');
		if(temps.length>0){
...
<body>
<p align="center"><s:form action="editorphan" method="post"
	theme="simple" enctype="multipart/form-data" onsubmit="return aaaa();">
	<s:token></s:token>
	...
	<s:hidden name="wubaohuDTO.gongyang" value="5"></s:hidden>
	<s:hidden name="wubaohuDTO.assistType" value=""></s:hidden>
	<s:hidden name="wubaohuDTO.mastername" value=""></s:hidden>
	<s:hidden name="wubaohuDTO.ssn" value=""></s:hidden>
	...
	<table width="600px" border="0" cellpadding="0" cellspacing="0"
		...
		<tr>
			...
			<td>来源</td>
			<td><s:select list="#{'1':'城市 ','2':'农村'}" listKey="key" listValue="value" id="ds" name="wubaohuDTO.ds"></s:select></td>
		</tr>
		<tr>
			<td>审批标识</td>
