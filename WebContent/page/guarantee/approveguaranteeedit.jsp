	...
	function getchilds(oid) {
		$.ajax({
			...
			success : function(json) {
				json = eval('(' + json + ')');
				var oexts = json['orgs'];
				var oSelect1 = $("#jizhongDTO_orgno")[0];
				var j = oSelect1.options.length;
				for ( var i = j; i >= 0; i--) {
					oSelect1.options.remove(0);
				}
				var oSelect = $("#jizhongDTO_orgno")[0];
				if (oexts.length > 0) {
...
<p align="center"><s:form action="editguarantee"
	...
	<s:token></s:token>
	<s:hidden id="wubaohuDTO.memberId" name="wubaohuDTO.memberId"></s:hidden>
	<s:hidden name="wubaohuDTO.ds"></s:hidden>
